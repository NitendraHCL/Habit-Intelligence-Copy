...
const PROVENANCE: DashboardProvenance = {
  kpis: {
    ...
    sources: [BASE_TABLE],
    logic:
      "Total Booked = SUM(total_booked_count) of agg_kpi rows across ALL stages (Completed + No Show + Pending + " +
      "Cancelled) in the filter window — every booked appointment, cancelled included; uses the same " +
      "cug/date/location/specialty/gender/age filters but drops the stage = 'Completed' restriction. " +
      "Cancelled = SUM(total_booked_count) for stage = 'Cancelled' (a subset, shown separately). " +
    ...
    sql: "Total Booked: SELECT SUM(total_booked_count), SUM(total_booked_count) FILTER (WHERE stage = 'Cancelled') AS cancelled FROM agg_kpi WHERE <all-stage filters>. Others: WITH per_uhid AS (… GROUP BY a.uhid) SELECT SUM(consult_count), COUNT(*), COUNT(*) FILTER (WHERE row_count >= 2).",
  },
  demographicSunburst: {
...
async function handler(request: NextRequest) {
  try {
    ...
    const bookedPromise = safeQuery(() => dwQuery<{ total_booked: string; cancelled: string }>(
      `SELECT COALESCE(SUM(a.total_booked_count), 0)::bigint AS total_booked,
    ...
    if (q.hasDateRange) {
      const yoyPrev = await safeQuery(() => dwQuery<{
        ...
        SELECT
          ...
          (SELECT COUNT(*) FROM per_uhid)::bigint AS unique_patients,
          (SELECT COUNT(*) FROM per_uhid WHERE row_count >= 2)::bigint AS repeat_patients,
          COALESCE((SELECT SUM(a.total_booked_count) FROM ${BASE_TABLE} a WHERE ${q.allStagePrevWhere}), 0)::bigint AS total_booked`,
        q.params
      ), "kpiYoY");
